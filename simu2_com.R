#########################################
### This file can be used to obtain the results of simulated data (compcodeR).
#########################################
###  Parameters (variable)  ###
...
###  Parameters (fixed)  ###
name = "com"                #package name for generating simulated data: name = {"tcc", "com", "pro"} (fixed)
...
set.seed(i)
comData <- generateSyntheticData(dataset = "compData",
                                 n.vars = G,
                                 samples.per.cond = n1,
                                 n.diffexp = PDEG*G,
...
truth <- comData@variable.annotations$differential.expression
data <- comData@count.matrix
data.cl <- comData@sample.annotations$condition
if(filtering == TRUE){
