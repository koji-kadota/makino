#########################################
### This file can be used to obtain the results of simulated data (PROPER).
#########################################
###  Parameters (variable)  ###
...
###  Parameters (fixed)  ###
name = "pro"                #package name for generating simulated data: name = {"tcc", "com", "pro"} (fixed)
...
simOpts <- RNAseq.SimOptions.2grp(ngenes = G,
                                  ...
                                  lOD = "bottomly",
                                  sim.seed = i)
proData <- simRNAseq(simOpts, n1, n2)

data <- proData$counts
data.cl <- proData$designs
colnames(data) <- c(paste('G1_rep', 1:n1, sep = ''), paste('G2_rep', 1:n2, sep = ''))
rownames(data) <- paste("gene_", 1:G, sep = "")

truth <- rep(0, nrow(data))
for(d in proData$DEid){
  truth[d] <- 1
}
